...
assert.strictEqual(provider.prefetchDepth("hey"), 1)
assert.strictEqual(provider.prefetchDepth("imap"), 0, "a TLS handshake per guess is not worth it")
...
assert.strictEqual(provider.can("hey", "label"), false, "HEY lists boxes as labels but cannot apply one")
assert.strictEqual(provider.can("imap", "label"), false)
